...
def insertar_flujo_firmas(doc_id, creador_id, tipo_donacion):
    ...
    # Verificar si ya existen registros para este documento
    cursor.execute("SELECT COUNT(*) FROM firmas_documento WHERE doc_id = ?", (doc_id,))
    if cursor.fetchone()[0] > 0:
        print(f"Ya existen registros de firma para el documento {doc_id}")
        conn.close()
        return
    ...
    try:
        for p in pasos:
            ...
            """, p)
        conn.commit()
        print(f"Flujo de firmas insertado correctamente para el documento {doc_id}")
    except sqlite3.Error as e:
        print(f"Error al insertar flujo de firmas: {e}")
        conn.rollback()
    finally:
        conn2.close()
        conn.close()
...
def get_documentos_para_firmar(user_id, rol):
    ...
    try:
        ...
        # Obtener documentos donde:
        # 1. El paso actual corresponde al rol del usuario
        # 2. Todos los pasos anteriores están firmados
        # 3. El paso actual está pendiente
        cursor_firmas.execute("""
            WITH DocumentosElegibles AS (
                SELECT f1.doc_id
                FROM firmas_documento f1
                WHERE (f1.user_id = ? OR f1.role = ?)
                AND f1.status = 'pendiente'
                AND NOT EXISTS (
                    SELECT 1
                    FROM firmas_documento f2
                    WHERE f2.doc_id = f1.doc_id
                    AND f2.orden < f1.orden
                    AND f2.status != 'firmado'
                )
            )
            SELECT DISTINCT f.doc_id
            FROM firmas_documento f
            JOIN DocumentosElegibles de ON f.doc_id = de.doc_id
            WHERE (f.user_id = ? OR f.role = ?)
        """, (user_id, rol, user_id, rol))
        ...
        if rol == "reception":
            ...
            # Inventario solo ve documentos de tipo especie
            query += " AND Type = 'especie'"
            params = doc_ids
        elif rol == "admin":
            # Admin ve todo
            params = doc_ids
        ...
        for row in rows:
            ...
            cursor_firmas.execute("""
                SELECT status, fecha_firma, orden
